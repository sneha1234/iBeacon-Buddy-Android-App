...
public class MyDrawableImageView extends ImageView{
	...
	// Called from the gps
	protected void makeUseOfNewLocation(float[] userpos) {
        MapCalibrator mn =new MapCalibrator();
       ...
       float[] userp =userpos;
        ...
        if(userp[0]!=0)
            System.out.println("userp[0]="+userp[0]);
	...
	//	m_SavableData.gpsToMapMapMatrix.mapPoints(mapCoordinate, gpsCoordinate);
		...
		float newLeft = (float) (userp[0] - m_SavableData.m_PositionMapPosition.width() / 2.0);
		float newTop = (float) (userp[1] - m_SavableData.m_PositionMapPosition.height() / 2.0);
    ...
    protected void makeUseOfNewLocation2() {
        MapCalibrator mn =new MapCalibrator();



        float userp[]= mn.userpos;
if(userp[0]!=0)
    System.out.println("userp[0]"+userp[0]);
        if (!m_SavableData.isTrackingPosition || m_SavableData.gpsToMapMapMatrix == null)
            return;
	...
	protected void setMapMatrix(Matrix mapMatrix) {
		...
		//	makeUseOfNewLocation(); //Draw out position on the map.
	}
